...
from src.metier.common.utils import ensure_list, parse_model_from_payload

ListNorm = Annotated[List[Any], BeforeValidator(ensure_list)]
...
class Auteurs(BaseModel):
    auteur: Annotated[List[Auteur], BeforeValidator(ensure_list)] = Field(
...
def parse_document_depuis_payload(data: Dict[str, Any]) -> Document:
    return parse_model_from_payload(data, Document, "document")
