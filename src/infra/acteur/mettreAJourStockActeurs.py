...
from src.metier.acteur.acteur import (
    ...
    Collaborateur as CollaborateurMetier,
    Mandat as MandatMetier,
    Suppleant as SuppleantMetier,
...
class MettreAJourStockActeurs(_BaseStockage):
    ...
    def _mettre_a_jour_stock_v2(self) -> int:
        ...
        with self.SessionLocal() as session:
            try:
                total_acteurs = self._enregistrer_acteurs_v2_depuis_dossier(session, batch_size=50)
                session.commit()
            except Exception:
                session.rollback()
                logger.exception(
                    "Rollback de la transaction en raison d'une erreur lors de la mise à jour des acteurs V2"
                )
                raise
        return total_acteurs

    def _enregistrer_acteurs_v2_depuis_dossier(
        self, session: SASession, batch_size: int = 50
    ) -> int:
        compteur_total = 0
        batch: list[ActeurMetier] = []
    ...
    def _creer_ou_mettre_a_jour_acteurs(
        self, session: SASession, acteurs: Iterable[ActeurMetier]
    ) -> None:
        for acteur in acteurs:
            self._creer_ou_mettre_a_jour_acteur(session, acteur)

    def _creer_ou_mettre_a_jour_acteur(
        self, session: SASession, acteur_metier: ActeurMetier
    ) -> None:
        ...
        acteur_v2 = session.get(ActeurV2, uid)
        if acteur_v2 is None:
            acteur_v2 = ActeurV2(uid=uid)
            session.add(acteur_v2)
        ...
        acteur_v2.civilite = _nettoyer_chaine(getattr(identite, "civ", None))
        acteur_v2.prenom = _nettoyer_chaine(getattr(identite, "prenom", None))
        acteur_v2.nom = _nettoyer_chaine(getattr(identite, "nom", None))
        acteur_v2.date_naissance = getattr(info_naissance, "dateNais", None)
        acteur_v2.ville_naissance = _nettoyer_chaine(getattr(info_naissance, "villeNais", None))
        acteur_v2.departement_naissance = _nettoyer_chaine(
            getattr(info_naissance, "depNais", None)
        )
        acteur_v2.pays_naissance = _nettoyer_chaine(getattr(info_naissance, "paysNais", None))
        acteur_v2.date_deces = None

        profession = acteur_metier.profession
        acteur_v2.profession_libelle = _nettoyer_chaine(
            getattr(profession, "libelleCourant", None)
        )
        soc_proc = getattr(profession, "socProcINSEE", None) if profession else None
        acteur_v2.categorie_socio_professionnelle = _nettoyer_chaine(
            getattr(soc_proc, "catSocPro", None)
        )
        acteur_v2.famille_socio_professionnelle = _nettoyer_chaine(
            getattr(soc_proc, "famSocPro", None)
        )
        acteur_v2.url_fiche_acteur = _nettoyer_chaine(
        ...
        acteur_v2.mandats.clear()
        for mandat in self._convertir_mandats(uid, acteur_metier.mandats.mandat if acteur_metier.mandats else []):
            acteur_v2.mandats.append(mandat)

    def _convertir_mandats(
        self, acteur_uid: str, mandats_metier: Iterable[MandatMetier]
    ) -> Iterable[MandatV2]:
        mandats_v2: list[MandatV2] = []
        for mandat_metier in mandats_metier:
            ...
            mandat_v2.collaborateurs.extend(self._convertir_collaborateurs(collaborateurs))
            mandat_v2.suppleants.extend(self._convertir_suppleants(suppleants))

            mandats_v2.append(mandat_v2)

        return mandats_v2

    def _convertir_collaborateurs(
        self, collaborateurs: Iterable[CollaborateurMetier]
    ) -> list[CollaborateurV2]:
        collaborateurs_v2: list[CollaborateurV2] = []
        for collaborateur in collaborateurs:
            collaborateurs_v2.append(
                CollaborateurV2(
        ...
        return collaborateurs_v2

    def _convertir_suppleants(
        self, suppleants: Iterable[SuppleantMetier]
    ) -> list[SuppleantV2]:
        suppleants_v2: list[SuppleantV2] = []
        for suppleant in suppleants:
            suppleants_v2.append(
                SuppleantV2(
